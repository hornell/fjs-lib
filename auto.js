...
(function(global) {
  ...
  function scanModules() {
    ...
    for (const key in moduleMap) {
      // check if any element has the class `key`
      const byClass = document.querySelector(`.${key}`);
      // or check if any element has attribute `key` (assuming attribute presence only, no value check)
      const byAttr = document.querySelector(`[${key}]`);

      if (byClass || byAttr) {
        needed.add(moduleMap[key]);
      }
    }
    return [...needed];
  }
